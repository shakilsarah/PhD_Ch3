...
ann <- ann[!(ann$Date %in% qgaps$year[qgaps$year!=2016]),]
mnth <- mnth[!(mnth$Date %in% qgaps$year[qgaps$year!=2016]),]
...
a <- a[a$rat < 1.0001 | a$year==2016,] 
# didn't catch 1997 because it moves into 1998
### ===== (3.2) Monthly =====
...
### ===== (3.1) Discharge =====
trenddis <- a[a$var=="tss", c("year", "diskm3yr")]

zyp_dis <- as.data.frame(
  zyp.yuepilon(
    trenddis$diskm3yr, trenddis$year, # need to figure out what method to use
...
zyp_poc <- as.data.frame(
            zyp.yuepilon(
              trendpoc$annualloadGMyr, trendpoc$year, # need to figure out what method to use
                 conf.intervals=TRUE, preserve.range.for.sig.test=TRUE
              )
)
...
zyp_doc <- as.data.frame(
  zyp.yuepilon(
    trenddoc$annualloadGMyr, trenddoc$year, # need to figure out what method to use
    conf.intervals=TRUE, preserve.range.for.sig.test=TRUE
  )
)
...
zyp_tss <- as.data.frame(
  zyp.yuepilon(
    trendtss$milltonnesyr, trendtss$year, # need to figure out what method to use
    conf.intervals=TRUE, preserve.range.for.sig.test=TRUE
  )
)
...
poctss <-  a[a$var=="tss"| a$var=="poc", c("year", "milltonnesyr", "var")]
poctss <- poctss %>% 
          pivot_wider(id_cols=year,
                      names_from=var,
                      values_from=milltonnesyr)

library(plotly)
ggplotly(
ggplot() + 
  geom_point(data=poctss,
             aes(x=tss, y=poc, colour=year))+
  geom_abline(slope=0.015)
)
##### ========== (3) Graph data ================================================
...
dis <- ggplot(data=a, 
       ...
       geom_point(size=2.5, shape=23, fill="grey", colour="black") + theme

#f$date2<-format(f$Date, format="%m-%d")
#dis <- ggplot(data=f, 
 #             aes(x=date2, y=Flow, group=year)) +
#  labs(y=expression("Annual Discharge (km"^3*" y"^-1*")"), x="month") +
 # geom_line(aes(colour=year)) + theme

## (3.3) Other variables ====================

docabline <- lm(annualloadGMyr~year, trenddoc)

oc <- ggplot(data=a[a$var=="doc"|a$var=="poc",], 
      aes(x=year, y=annualloadGMyr)) +
      geom_smooth(data=trenddoc, aes(x=year, y=annualloadGMyr),
                  method='lm',formula=y~x, se=FALSE, colour="black") +   
      geom_point(aes(fill=var, shape=var), size=2.5, 
                  colour="black") +
     #  geom_abline(slope=0.1865268, intercept=-323) +
       labs(y=expression("Annual OC Load (Gmol y"^-1*")"), x="Year") +
  scale_fill_manual(values=c("Sky Blue", "orange")) +
  scale_shape_manual(values=c(23, 22)) +
  theme 


t <- ggplot(data=a[a$var=="tss",],
             aes(x=year, y=milltonnesyr)) +
    geom_point(size=2.5, fill="grey", shape=23) +
...
# 
pt <- a %>%
  select(var, milltonnesyr, year) %>%
  pivot_wider(names_from=var,
              values_from=milltonnesyr)


ggplot(pt, aes(x=tss, y=poc)) + geom_label(aes(label=year)) +
  geom_abline(intercept=0, slope=0.01, colour="black") +
  theme_classic()

t <- lm(poc~tss, data=pt)
plot(t)

pt$poctss <- pt$poc/pt$tss

ggplot(pt, aes(x=year, y=poctss)) + geom_point() + geom_hline(yintercept=0.01)
...
savePlotpdf <- function(myPlot, w, h, filename) {
  pdf(file = paste0(wd, "ThesisDrafts/Chapter3/Graphs/Fig6/", filename, ".pdf"),
      width = w, height = h)
  print(myPlot)
  dev.off()
}
...
savePlotpdf(grid.draw(cbind(ggplotGrob(dis),
                            ggplotGrob(oc), 
                            ggplotGrob(t), size="first")
                      ),
                      7.5, 2.5, "annual")
